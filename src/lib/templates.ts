// Email templates as hand-written HTML strings.
//
// React Email components live in src/emails/ for any future migration done
// via @react-email/render's async API. We don't use them at runtime because
// Next 14 forbids react-dom/server from leaking into the route tree, and the
// async render() requires turning every email-sending callsite into an async
// chain. Hand-written HTML is sync, dependency-free, and the existing tests
// cover XSS escaping.
...
export function welcomeEmail(v: WelcomeEmailVars): { subject: string; html: string } {
  ...
  const inner = `
    ...
    <p>It takes effort to get people&rsquo;s attention these days, so know this &mdash; we really appreciate and value you taking the time to be part of <strong>${escapeHtml(env.NEXT_PUBLIC_SITE_NAME)}</strong>. We&rsquo;ll make every effort to be efficient and effective with our communication.</p>
    ...
    <p>Looking forward to meeting in person at the next event.<br/>Kind regards,<br/>The ${escapeHtml(env.NEXT_PUBLIC_SITE_NAME)} team</p>
  `;

  return {
    subject: `Welcome to ${env.NEXT_PUBLIC_SITE_NAME}`,
    html: layout(`Welcome to ${env.NEXT_PUBLIC_SITE_NAME}`, inner, v.preferencesUrl, v.unsubscribeUrl)
  };
}
...
export function announcementEmail(v: AnnouncementEmailVars): { subject: string; html: string } {
  ...
  return {
    ...
    html: layout(v.eventTitle, inner, v.preferencesUrl, v.unsubscribeUrl)
  };
}
